...
float MpfrCalculateLog(float x) {
...
float myLogTest(float x, unsigned long long* time) {
    unsigned int dummy;
...
float mlibLogTest(float x, unsigned long long* time) {
    unsigned int dummy;
    unsigned long long t1 = __rdtscp(&dummy);
    float result = logf(x);
    unsigned long long t2 = __rdtscp(&dummy);
...
float doubleLogTest(float x, unsigned long long* time) {
    unsigned int dummy;
    unsigned long long t1 = __rdtscp(&dummy);
    float result = log((double)x);
    unsigned long long t2 = __rdtscp(&dummy);
    *time += (t2 - t1);
    return result;
}
...
int main(int argc, char** argv) {
    ...
    for (count = 0x0; count < 0x100000000; count++) {
        ...
        float bres = myLogTest(x, &myTime);
        float bmy = MpfrCalculateLog2(x);
        float bfy = mlibLogTest(x, &mlibTime);
        float bdy = doubleLogTest(x, &doubleTime);

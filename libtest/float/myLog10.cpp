...
float MpfrCalculateLog10(float x) {
...
float myLog10Test(float x, unsigned long long* time) {
    unsigned int dummy;
    unsigned long long t1 = __rdtscp(&dummy);
    float result = mylog10(x);
    unsigned long long t2 = __rdtscp(&dummy);
    *time += (t2 - t1);
    return result;
}

float mlibLog10Test(float x, unsigned long long* time) {
    unsigned int dummy;
    unsigned long long t1 = __rdtscp(&dummy);
...
float doubleLog10Test(float x, unsigned long long* time) {
    unsigned int dummy;
    unsigned long long t1 = __rdtscp(&dummy);
    float result = log10((double)x);
    unsigned long long t2 = __rdtscp(&dummy);
    *time += (t2 - t1);
    return result;
}
...
int main(int argc, char** argv) {
    ...
    for (count = 0x0; count < 0x100000000; count++) {
        ...
        float bres = myLog10Test(x, &myTime);
        float bmy = MpfrCalculateLog10(x);
        float bfy = mlibLog10Test(x, &mlibTime);
        float bdy = doubleLog10Test(x, &doubleTime);
